...
| Engine (stdlib-only Python ≥ 3.8 syntax, suite exercised on 3.10–3.13, no third-party packages) | `scripts/log-triage/` (`python3 scripts/log-triage …`) |
...
* **Cancellation vs timeout.** .NET `OperationCanceledException`/`TaskCanceledException` are classified as observed
  timeouts (that is how HttpClient/ADO.NET report them); a cancellation triggered by the caller is misreported as a
  timeout.
* **Samples** under `docs/log-triage/samples/` were produced from copies of the fixtures plus two throwaway Git
  repositories (see `samples/README.md`); they validate against the schema but are not regenerated by CI.
