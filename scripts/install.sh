...
echo "  /plugin marketplace add tomas-rampas/agentic-framework"
echo "  /plugin install agentic-framework@agentic-framework"
echo "  (optional) /plugin install agentic-framework-mcp@agentic-framework"
...
command -v uvx >/dev/null 2>&1 || echo "WARNING: uvx (uv) not found - the serena/fetch/code-review-graph MCP servers need it." >&2
