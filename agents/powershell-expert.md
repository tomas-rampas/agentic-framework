...
- Under the framework's selective policy, callers run short commands inline and route only long, output-heavy work to an executor agent — test batteries, log grinding, paged enumerations, multi-step throwaway pipelines. If a small request does arrive, handle it crisply: run, report the four-part contract, done. Encourage batching — related commands arrive best as one request answered by one report.
