...
The six launchers are unpinned, so each session start resolves the newest published release from npm (filesystem, context7, sequential-thinking) or PyPI/GitHub (serena, fetch, code-review-graph). Downloads happen on the first run after a fresh install, a cleared `uv`/`npm` cache, or when an upstream release publishes a new version — an unpinned serena re-resolves its git repository at each launch. A new upstream release therefore runs at your next session start before anyone has reviewed it; the controls are the ones described in this section and the `ask` rule for `apply_refactor_tool` (Installation step 3). Cost note: serena is the bundle's session-start heavyweight (it boots language servers for the project) and code-review-graph is its per-request heavyweight (30 tools, about 37 KB of tool schema advertised to every session); if you don't use one of them, switch it off in the `/mcp` panel, or add its scoped name (`plugin:agentic-framework:<server>`) to that project's `disabledMcpServers` list in `~/.claude.json` — see **About code-review-graph** below for the exact shape. Do not edit the plugin cache's `.mcp.json` — `claude plugin update` overwrites it.
...
- A user-scope server in `~/.claude.json` with a name like `serena` or `context7` **shadows** the plugin's copy. Delete those entries to get the servers the plugin ships.
