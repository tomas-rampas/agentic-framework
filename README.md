...
- **Real Enforcement Hooks** — a blocking peer-review Stop gate and a model-tier guard on sub-agent calls, plus session-context and delegation-hint hooks, registered via the plugin's `hooks/hooks.json` and covered by tests
...
The six launchers are unpinned: the manifest fixes no version, so new upstream code arrives without a plugin update. Measured 2026-09-18 on a warm cache: `npx` revalidates the package against the npm registry at every launch (filesystem, context7, sequential-thinking); `uvx` re-resolves the requirement at every launch and reuses its downloads when they are still the newest (fetch, code-review-graph); serena is different again — its source is a git URL with no ref, so each launch asks GitHub for the commit at the repository's default branch and runs that, an unreviewed push rather than a release. Whatever resolves is downloaded and executed with your user privileges before anyone has reviewed it, and a launch that cannot reach the registry or GitHub may fail. The controls this plugin documents that act before that code runs are not connecting the server (per-project `disabledMcpServers`, or the `/mcp` panel for later sessions) and a same-named server in your own user or project scope, which outranks the plugin's entry and is the supported way to run a version you pinned yourself; `CRG_TOOLS` and the `ask` rule for `apply_refactor_tool` (Installation step 3) narrow what a running server may do, not what it runs at launch. Cost note: serena is the bundle's session-start heavyweight (it boots language servers for the project) and code-review-graph is its per-request heavyweight (30 tools and about 37 KB of tool schema at 2.3.8, advertised to every session); if you don't use one of them, switch it off in the `/mcp` panel, or add its scoped name (`plugin:agentic-framework:<server>`) to that project's `disabledMcpServers` list in `~/.claude.json` — see **About code-review-graph** below for the exact shape. Do not edit the plugin cache's `.mcp.json` — `claude plugin update` overwrites it.

serena is launched with `--project-from-cwd`, so it activates the repository Claude Code was started in and registers it in serena's global config (`~/.serena/serena_config.yml`) on first run, creating a `.serena/project.yml` file in the repo. Started outside a git repository, it logs a warning and starts with no project activated. Three consequences to know before you open a repository you did not write. First, activation is now automatic: serena reads the repository's own `.serena/project.yml` at server launch, before the first model turn and with no tool call in the transcript, and boots language servers for the project's languages (some language servers evaluate project build configuration). Second, that file may carry an `activation_command`, which serena executes with your privileges in the project root when — and only when — the project path matches `trusted_project_path_patterns` in `~/.serena/serena_config.yml`. Upstream ships that list empty, which makes a committed `activation_command` inert; keep it empty or narrow it to directories you own, and never set it to `**` on a machine where you clone other people's code. Third, serena's generated `.serena/.gitignore` covers only `/cache` and `/project.local.yml`, so `.serena/project.yml` and `.serena/memories/` show up as untracked files in your repository: commit them deliberately (they are meant to be shared) or ignore `.serena/` yourself. Inside a repository serena now runs in single-project mode: it does not expose `activate_project` or `get_current_config` (measured: 21 tools instead of 23), so an agent cannot switch to a different project root mid-session — a sibling worktree or a monorepo sub-project needs its own session. Outside a repository both tools are present. To opt out of automatic activation, define your own `serena` server in user or project scope without `--project-from-cwd`; it outranks the plugin's entry.

**About code-review-graph — read this before your first session.** Installing the framework connects this server automatically, with no approval prompt, and it ships with its full tool set, which is not read-only. `apply_refactor_tool` writes source files into whatever directory the model passes as `repo_root` — every tool that targets a single repository accepts that parameter (28 of the 30), an explicit value overrides the `--repo` launch flag, and the package's only guard on that root is that the target contains a `.git`, `.svn`, or `.code-review-graph` directory, so any checkout on your machine is reachable. `cross_repo_search_tool` / `list_repos_tool` read any other repositories registered with the tool. It is also a solo-maintained community package (MIT, no PyPI attestations as of 2.3.8). The tool counts and the root guard above were measured against 2.3.8; the manifest is unpinned, so verify them against the version you resolve. If that is not acceptable on your machine, switch it off before first use. Edit `~/.claude.json` before you start a session and disable the server under the entry for the project you are about to open:
...
Its tools return nothing until a graph exists — call `build_or_update_graph_tool` once, or run `uvx code-review-graph build` in the repo (the CLI and the server should run the same package version so they agree on the database schema; after an upgrade, run `uvx --refresh code-review-graph build` to realign them). The graph is written to `<repo>/.code-review-graph/` in every repository you point it at (the package gitignores that directory); `CRG_DATA_DIR` relocates it.
...
- A user-scope server in `~/.claude.json` with a name like `serena` or `context7` **shadows** the plugin's copy. Delete those entries to get the servers the plugin ships, or keep them deliberately: a same-named entry in your own user or project scope shadows the plugin's unpinned launcher, which is the supported way to run a version you pinned yourself.
...
The most capable model orchestrates; the cheapest model that can do the job does the job. Every agent carries a default tier in its frontmatter (`model:`), mirrored in `claude.json` and in the `Default tier` column of the agent table in `CLAUDE.md`: review gates, security and architecture run on `opus`; language experts, domain specialists, analysis and planning on `sonnet`; executors, docs and UI/UX on `haiku`. The orchestrating session chooses the model on every delegation through the Agent tool's `model` parameter — one tier down for mechanical work, one tier up (with the reason stated) for concurrency, auth/crypto, large refactors or after two failed review rounds — and never runs a sub-agent on its own top tier. Built-in agents (`Explore`, `Plan`, `general-purpose`, `claude`) have no default tier and would inherit the parent's model, so they always get an explicit one: the orchestrator passes it, and when it does not, the `pretooluse-model-guard` hook fills it in automatically (`haiku` for `Explore`, `sonnet` for the others) with nothing to configure (on POSIX hosts the hook needs `jq`, like every hook here; without it the hook enforces nothing). The same hook denies the top tier and every `fork`. Setting `CLAUDE_CODE_SUBAGENT_MODEL=sonnet` in your settings `env` is optional: it additionally covers agents the hook cannot recognise (your own, or another plugin's, when their definition has no `model:`). Rationale and measurements: [`docs/design/model-tiering.md`](docs/design/model-tiering.md).
...
| `pretooluse-model-guard` (.ps1/.sh pair) | `PreToolUse` (`Task\|Agent`) | **Blocking and rewriting.** The model-tier ceiling, automatic with nothing to configure (POSIX hosts need `jq`, or the hook enforces nothing): denies a sub-agent call whose `model` names the top tier (`fable`, in any spelling or as a full model id), denies every `fork` (a fork always runs on its parent's model), and **rewrites** a built-in agent call (`Explore`, `Plan`, `general-purpose`, `claude`, or no type) that carries no `model` — it sets `haiku` for `Explore` and `sonnet` for the rest, echoes every other field back unchanged, and lets the call proceed without touching your permission rules (when `CLAUDE_CODE_SUBAGENT_MODEL` is set to a usable tier the hook stays out of the way and the variable decides). It is not a general ceiling: an agent type the hook does not know (your own, or another plugin's) is neither denied nor rewritten, because its frontmatter is invisible to the hook; if that definition has no `model:` it inherits the caller's tier, and `CLAUDE_CODE_SUBAGENT_MODEL` is the cover. Fail-open on any payload it cannot parse; `AF_MODEL_GUARD=off` disables it. |
...
**Built for Claude Code CLI • 21 Specialized Agents • 5 Hook Scripts • 9 Skills • 12 Commands • v4.5.1**
