...
| `pretooluse-model-guard` (.ps1/.sh pair) | `PreToolUse` (`Task\|Agent`) | **Blocking.** The model-tier ceiling: denies a sub-agent call whose `model` names the top tier (`fable`, in any spelling or as a full model id), denies every `fork` (a fork always runs on its parent's model), and denies a built-in agent call (`Explore`, `Plan`, `general-purpose`, `claude`, or no type) that carries no `model` while `CLAUDE_CODE_SUBAGENT_MODEL` is unset or itself names the top tier — the denial names the tier to pass. It is not a general ceiling: an agent type the hook does not know (your own, or another plugin's) is never denied, because its frontmatter is invisible to the hook; if that definition has no `model:` it inherits the caller's tier, and `CLAUDE_CODE_SUBAGENT_MODEL` is the cover. Fail-open on any payload it cannot parse; `AF_MODEL_GUARD=off` disables it. |
