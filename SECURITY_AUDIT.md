...
- **MCP server configuration**: `.mcp.json` at the plugin root (shipped in the agentic-framework plugin)
...
1. **Environment variable parameterization** — secrets are consumed via `${VAR}` expansion at runtime, never committed. Example: the plugin's root `.mcp.json` wires `"CONTEXT7_API_KEY": "${CONTEXT7_API_KEY:-}"` from the environment at server launch.
...
6. **MCP server secrets** — runtime references (e.g., `${CONTEXT7_API_KEY}` in the plugin's root `.mcp.json`) are never expanded by the installer; placeholders resolve at server launch time from the live environment.
